...
function CustomerFormModal({
  ...
  function handleEnterToNext(event: KeyboardEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) {
    ...
    const submitButton = form.querySelector<HTMLButtonElement>('#save-customer-button')
    submitButton?.focus()
  }
          ...
          <form onSubmit={handleSubmit(onSubmit)} style={{ padding: '0 32px 32px 32px', display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 26 }}>
                ...
                />
                <label htmlFor="customer-is-active" style={{ fontSize: 14, color: 'var(--color-text-primary)', cursor: 'pointer' }}>
                  Customer is Active
                </label>
              </div>
            </div>
                ...
                <tr key={customer.id}>
                  ...
                  <td className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <span style={{ color: 'var(--color-text-primary)' }}>{customer.contactName}</span>
                      <span style={{ fontSize: 11, color: 'var(--color-text-dim)' }}>{customer.phone}</span>
                    </div>
                  </td>
                  <td className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    {customer.city}
                  ...
                  <td className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    {CUSTOMER_GROUPS.find((group) => group.id === customer.customerGroupId)?.name ?? customer.customerGroupId}
                  </td>
                  <td className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    {TERRITORIES.find((territory) => territory.id === customer.territoryId)?.name ?? customer.territoryId}
                  </td>
                  <td className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                    {ROUTES.find((route) => route.id === customer.routeId)?.name ?? customer.routeId}
                  </td>
                  <td className="mono text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    {customer.creditDays}
                  </td>
                  <td className="mono text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    {customer.creditLimit.toLocaleString('en-LK', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
                  <td>
