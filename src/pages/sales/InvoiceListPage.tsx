...
import { Eye, Plus, Search, X, Pencil } from 'lucide-react'
...
function deriveDisplayStatus(inv: any) {
...
function InvoiceFormModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      setTimeout(() => {
        const firstInput = document.querySelector<HTMLElement>('select[name="customerId"]')
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      ...
      ).filter((el) => !el.hasAttribute('disabled') && el.tabIndex !== -1 && !el.hasAttribute('data-skip-focus'))
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(v) => !v && onClose()}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50" style={{ background: 'rgba(0,4,12,0.75)', backdropFilter: 'blur(2px)' }} />
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full -translate-x-1/2 -translate-y-1/2 shadow-2xl" style={{ maxWidth: 500, background: 'var(--color-bg-surface)', border: '1px solid var(--color-border)', borderRadius: 12 }}>
          <div style={{ padding: '32px 32px 24px 32px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
              <Dialog.Title style={{ fontSize: 22, fontWeight: 600, color: 'var(--color-text-primary)' }}>{invoice ? 'Edit Invoice' : 'Create Draft Invoice'}</Dialog.Title>
              <Dialog.Description style={{ marginTop: 8, fontSize: 13, color: 'var(--color-text-muted)' }}>
                {invoice ? 'Update the details for this invoice.' : 'Initialize a new draft invoice. Line items can be added on the detail page.'}
              </Dialog.Description>
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} style={{ padding: '0 32px 32px 32px', display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, letterSpacing: '0.8px', color: 'var(--color-text-muted)', marginBottom: 8, textTransform: 'uppercase' }}>CUSTOMER</label>
              <select className={`form-input ${errors.customerId ? 'error' : ''}`} style={{ width: '100%', height: 44, background: 'rgba(0,0,0,0.15)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '0 16px', color: 'var(--color-text-primary)', fontSize: 14, cursor: 'pointer' }} autoFocus onKeyDown={handleKeyDown} {...register('customerId')}>
                <option value="" disabled style={{ background: 'var(--color-bg-elevated)', color: 'var(--color-text-dim)' }}>Select a customer...</option>
                {mockCustomers.map(c => <option key={c.id} value={c.id} style={{ background: 'var(--color-bg-elevated)', color: 'var(--color-text-primary)' }}>{c.name}</option>)}
              </select>
              {errors.customerId && <p className="form-error mt-1" style={{ fontSize: 12, color: 'var(--color-danger)' }}>{errors.customerId.message}</p>}
            </div>

            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, letterSpacing: '0.8px', color: 'var(--color-text-muted)', marginBottom: 8, textTransform: 'uppercase' }}>INVOICE DATE</label>
                <input type="date" className={`form-input ${errors.invoiceDate ? 'error' : ''}`} style={{ width: '100%', height: 44, background: 'rgba(0,0,0,0.15)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '0 16px', color: 'var(--color-text-primary)', fontSize: 14, cursor: 'pointer' }} onKeyDown={handleKeyDown} {...register('invoiceDate')} />
                {errors.invoiceDate && <p className="form-error mt-1" style={{ fontSize: 12, color: 'var(--color-danger)' }}>{errors.invoiceDate.message}</p>}
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, letterSpacing: '0.8px', color: 'var(--color-text-muted)', marginBottom: 8, textTransform: 'uppercase' }}>DUE DATE</label>
                <input type="date" className={`form-input ${errors.dueDate ? 'error' : ''}`} style={{ width: '100%', height: 44, background: 'rgba(0,0,0,0.15)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '0 16px', color: 'var(--color-text-primary)', fontSize: 14, cursor: 'pointer' }} onKeyDown={handleKeyDown} {...register('dueDate')} />
                {errors.dueDate && <p className="form-error mt-1" style={{ fontSize: 12, color: 'var(--color-danger)' }}>{errors.dueDate.message}</p>}
              </div>
            </div>

            <div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, letterSpacing: '0.8px', color: 'var(--color-text-muted)', marginBottom: 8, textTransform: 'uppercase' }}>PAYMENT TYPE</label>
              <select className={`form-input ${errors.paymentType ? 'error' : ''}`} style={{ width: '100%', height: 44, background: 'rgba(0,0,0,0.15)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '0 16px', color: 'var(--color-text-primary)', fontSize: 14, cursor: 'pointer' }} onKeyDown={handleKeyDown} {...register('paymentType')}>
                <option value={PaymentType.Cash} style={{ background: 'var(--color-bg-elevated)' }}>Cash</option>
                <option value={PaymentType.Credit} style={{ background: 'var(--color-bg-elevated)' }}>Credit</option>
              </select>
            </div>
            ...
            <div>
              ...
              <textarea className={`form-input ${errors.notes ? 'error' : ''}`} style={{ width: '100%', minHeight: 80, background: 'rgba(0,0,0,0.15)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '12px 16px', color: 'var(--color-text-primary)', fontSize: 14, resize: 'vertical' }} placeholder="E.g. Delivery requested in the morning." onKeyDown={handleKeyDown} {...register('notes')} />
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 8 }}>
              <button type="button" className="button-secondary" onClick={onClose} data-skip-focus="true" style={{ height: 40, padding: '0 24px', fontSize: 14 }}>Cancel</button>
              <button type="submit" className="button-primary" onKeyDown={handleKeyDown} style={{ height: 40, padding: '0 24px', fontSize: 14 }}>{invoice ? 'Save Changes' : 'Create Draft Invoice'}</button>
            </div>
          </form>
        </Dialog.Content>
      </Dialog.Portal>
...
export default function InvoiceListPage() {
  ...
  const handleInvoiceSaved = (savedInv: any) => {
  ...
  const openEditInvoiceModal = (inv: any, e: React.MouseEvent) => {
    e.stopPropagation()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="panel" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          >
            {TYPE_OPTIONS.map((t) => (
              <option key={t} value={t} style={{ background: 'var(--color-bg-elevated)', color: 'var(--color-text-primary)' }}>
                {t === 'All' ? 'All Types' : t}
              </option>
            ))}
          </select>
                ...
                return (
                  <tr key={inv.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/sales/invoices/${inv.id}`)}>
                    ...
                    <td onClick={(e) => e.stopPropagation()}>
                      <button className="icon-button" title="Edit invoice" style={{ width: 28, height: 28 }} onClick={(e) => openEditInvoiceModal(inv, e)}>
