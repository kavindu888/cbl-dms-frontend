...
import { firstValidationMessage, mustBeDifferent, positiveNumber, required, nonNegativeNumber } from '@/utils/validation'
...
const pageTitles = {
  ...
  transfers: {
    ...
    subtitle: 'Create transfers, add lines, dispatch, receive, or cancel stock movement between locations.',
  },
  stocktakes: {
    title: 'Stocktakes',
  ...
  movements: {
    ...
    subtitle: 'Review stock movement history from receipts, sales, returns, transfers, and adjustments.',
  },
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', ...style }}>
      <label className="form-label" style={{ fontSize: 10 }}>
        ...
        {required ? (
          ...
          <span style={{ color: 'var(--color-text-dim)', fontWeight: 'normal', textTransform: 'none', marginLeft: 4 }}>
            (Optional)
          </span>
        )}
      </label>
...
export default function StockModulePage({ initialTab = 'levels' }) {
  ...
  const [movementFilters, setMovementFilters] = useState({ productId: '', stockLocationId: '', movementType: '', from: '', to: '' })
  ...
  const loadLevels = useCallback(async () => {
    ...
    setError('')
    try {
      const params = { stockLocationId: levelFilters.stockLocationId || undefined, lowStockOnly: levelFilters.lowStockOnly || undefined }
  ...
  useEffect(() => { loadCommonData() }, [loadCommonData])
  useEffect(() => {
    setStatusFilter('')
    setError('')
    setLookupProductId('')
  ...
  function editLocation(location) {
    ...
    setLocationForm({ businessUnitId: location.businessUnitId || '', code: location.code || '', name: location.name || '', address: location.address || '' })
  }
  ...
  async function saveLocation(event) {
    ...
    const validationMessage = firstValidationMessage([
      ...
      required(!editingLocation ? locationForm.businessUnitId : 'selected', 'Business unit is required.'),
    ...
    try {
      if (editingLocation) {
        await inventoryService.updateStockLocation(editingLocation.id, { name: locationForm.name.trim(), address: locationForm.address.trim() || null })
        toast.success('Stock location updated.')
      } else {
        await inventoryService.createStockLocation({ businessUnitId: locationForm.businessUnitId, code: locationForm.code.trim().toUpperCase(), name: locationForm.name.trim(), address: locationForm.address.trim() || null })
  ...
  async function createTransfer(event) {
    ...
    const validationMessage = firstValidationMessage([
      ...
      mustBeDifferent(transferForm.sourceLocationId, transferForm.destinationLocationId, 'Source and destination must be different.'),
    ])
    if (validationMessage) return toast.error(validationMessage)

    setIsSaving(true)
    try {
      const id = await inventoryService.createStockTransfer({ sourceLocationId: transferForm.sourceLocationId, destinationLocationId: transferForm.destinationLocationId, notes: transferForm.notes.trim() || null })
  ...
  async function addTransferLine(event) {
    ...
    setIsSaving(true)
    try {
      await inventoryService.addStockTransferLine(transferLineForm.transferId, { productId: transferLineForm.productId, productSku: transferLineForm.productSku.trim(), sourceBatchId: transferLineForm.sourceBatchId.trim(), qty })
  ...
  async function createStocktake(event) {
    ...
    setIsSaving(true)
    try {
      const id = await inventoryService.createStocktake({ stockLocationId: stocktakeForm.stockLocationId, notes: stocktakeForm.notes.trim() || null })
  ...
  async function addStocktakeLine(event) {
    ...
    setIsSaving(true)
    try {
      await inventoryService.addStocktakeLine(stocktakeLineForm.stocktakeId, { productId: stocktakeLineForm.productId, productSku: stocktakeLineForm.productSku.trim() })
      toast.success('Stocktake line added.')
      setStocktakeLineForm({ ...emptyStocktakeLineForm, stocktakeId: stocktakeLineForm.stocktakeId })
  ...
  function LocationSelect({ value, onChange, emptyLabel = 'Select location' }) {
    return (
      <SelectControl>
        <select className="form-input" value={value} onChange={(event) => onChange(event.target.value)} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
          <option value="">{emptyLabel}</option>
          {activeLocations.map((location) => (
            <option key={location.id} value={location.id}>{locationName(location.id)}</option>
          ))}
        </select>
      </SelectControl>
    )
      ...
      <SelectControl>
        <select className="form-input" value={value} onChange={(event) => onChange(event.target.value)} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
          ...
          {transfers.map((transfer) => (
            <option key={transfer.id} value={transfer.id}>{transfer.transferNo} - {transfer.status}</option>
          ))}
        </select>
      </SelectControl>
    )
      ...
      <SelectControl>
        <select className="form-input" value={value} onChange={(event) => onChange(event.target.value)} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
          ...
          {stocktakes.map((stocktake) => (
            <option key={stocktake.id} value={stocktake.id}>{stocktake.stocktakeNo} - {stocktake.status}</option>
          ))}
        </select>
      </SelectControl>
    )
  ...
  return (
    <div className="responsive-page" style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--color-text-primary)' }}>{pageTitles[activeTab]?.title || 'Stock Management'}</h1>
          <p style={{ marginTop: 4, color: 'var(--color-text-muted)', fontSize: 13 }}>{pageTitles[activeTab]?.subtitle || 'Manage inventory stock operations.'}</p>
        </div>
  ...
  function renderLevels() {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0 }}>
        <div className="panel responsive-filter-bar" style={{ padding: 14, display: 'grid', gridTemplateColumns: '260px 160px 1fr', gap: 12 }}>
          <SelectControl>
            <select className="form-input" value={levelFilters.stockLocationId} onChange={(event) => setLevelFilters((current) => ({ ...current, stockLocationId: event.target.value }))} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
              <option value="">All locations</option>
              {locations.map((location) => <option key={location.id} value={location.id}>{locationName(location.id)}</option>)}
            </select>
          </SelectControl>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--color-text-muted)', fontSize: 13 }}>
            <input type="checkbox" checked={levelFilters.lowStockOnly} onChange={(event) => setLevelFilters((current) => ({ ...current, lowStockOnly: event.target.checked }))} />
            Low stock only
          </label>
          <SelectControl>
            <select className="form-input" value={lookupProductId} onChange={(event) => setLookupProductId(event.target.value)} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
        ...
        {availability ? (
          <div className="panel" style={{ padding: 14, display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr 1fr 1.3fr', gap: 16 }}>
            <Metric label="Product ID" value={availability.productId} isMono />
            <Metric label="Product SKU" value={availability.productSku || '-'} />
            <Metric label="Available" value={formatNumber(availability.totalAvailable)} />
            <Metric label="Reserved" value={formatNumber(availability.totalReserved)} />
            <Metric label="Sellable" value={formatNumber(availability.sellable)} />
        ...
        <div className="panel responsive-table-shell" style={{ padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
          <div style={{ overflow: 'auto', flex: 1 }}>
            <table className="data-table master-table-compact">
              <thead><tr><th>ID</th><th>Product ID</th><th>Product SKU</th><th>Location ID</th><th>Location</th><th>Available</th><th>Reserved</th><th>Sellable</th><th>Last Movement</th></tr></thead>
              <tbody>
                {isLoading ? <LoadingRow colSpan={9} message="Loading stock levels..." /> : null}
                {!isLoading && error ? <ErrorRow colSpan={9} message={error} /> : null}
                {!isLoading && !error && levels.length === 0 ? <EmptyRow colSpan={9} message="No stock levels found." /> : null}
                {!isLoading && !error ? levels.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="amount-primary">{formatNumber(item.totalAvailable)}</td>
                    <td className="amount">{formatNumber(item.totalReserved)}</td>
                    <td className="amount-success">{formatNumber(item.sellable)}</td>
  ...
  function renderLocations() {
    return (
      <div className="responsive-master-detail" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 360px', gap: 16, flex: 1, minHeight: 0 }}>
        <div className="panel responsive-table-shell" style={{ padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ overflow: 'auto', flex: 1, marginBottom: 12 }}>
            <table className="data-table master-table-compact">
              <thead><tr><th>ID</th><th>Code</th><th>Name</th><th>Business Unit ID</th><th>Business Unit</th><th>Active</th><th>Status</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
              <tbody>
                {isLoading ? <LoadingRow colSpan={8} message="Loading stock locations..." /> : null}
                {!isLoading && error ? <ErrorRow colSpan={8} message={error} /> : null}
                {!isLoading && !error && locations.length === 0 ? <EmptyRow colSpan={8} message="No stock locations found." /> : null}
                {!isLoading && !error ? pagedLocations.map((location) => (
                  <tr key={location.id}>
                    <td className="mono">{location.id}</td>
                    <td><span className="mono" style={{ color: 'var(--color-amber)' }}>{location.code}</span></td>
                    <td>{location.name}</td>
                    ...
                    <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                      {canManageWarehouses ? (
                        <>
                          ...
                          <SmallButton onClick={() => changeLocationStatus(location)}>{location.isActive ? 'Deactivate' : 'Activate'}</SmallButton>
                        </>
                      ) : '-'}
                    </td>
                  </tr>
        ...
        {canManageWarehouses ? (
          <form className="panel" onSubmit={saveLocation} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
            <h2 style={{ fontSize: 16, fontWeight: 650 }}>{editingLocation ? 'Edit Location' : 'New Location'}</h2>
            <Field label="Business Unit" required>
              <SelectControl>
                <select className="form-input" value={locationForm.businessUnitId} disabled={Boolean(editingLocation)} onChange={(event) => setLocationField('businessUnitId', event.target.value)} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
            ...
            <Field label="Code" required>
              <input className="form-input" value={locationForm.code} disabled={Boolean(editingLocation)} placeholder="e.g. WH-01" onChange={(event) => setLocationField('code', event.target.value)} />
            </Field>
            <Field label="Name" required>
              <input className="form-input" value={locationForm.name} placeholder="Main Warehouse" onChange={(event) => setLocationField('name', event.target.value)} />
            </Field>
            <Field label="Address" style={{ flex: 1 }}>
              <textarea className="form-input" value={locationForm.address} placeholder="Optional address" onChange={(event) => setLocationField('address', event.target.value)} style={{ flex: 1, minHeight: 90, paddingTop: 10, resize: 'none' }} />
            </Field>
            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
              <button type="button" className="button-ghost" onClick={clearLocationForm} style={{ flex: 1 }}>Clear</button>
              <button type="submit" className="button-primary" disabled={isSaving} style={{ flex: 1 }}>{isSaving ? 'Saving...' : 'Save'}</button>
            </div>
          </form>
        ) : (
          <PermissionNotice>Warehouse management permission is required to create, edit, activate, or deactivate stock locations.</PermissionNotice>
        )}
      </div>
    )
  }
  ...
  function renderMovements() {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0 }}>
        <div className="panel responsive-filter-bar" style={{ padding: 14, display: 'grid', gridTemplateColumns: '1fr 230px 180px 170px 170px', gap: 12 }}>
          <ProductSelect value={movementFilters.productId} onChange={(value) => setMovementFilters((current) => ({ ...current, productId: value }))} emptyLabel="All products" />
          <LocationSelect value={movementFilters.stockLocationId} onChange={(value) => setMovementFilters((current) => ({ ...current, stockLocationId: value }))} emptyLabel="All locations" />
          <SelectControl>
            <select className="form-input" value={movementFilters.movementType} onChange={(event) => setMovementFilters((current) => ({ ...current, movementType: event.target.value }))} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
          ...
          <input className="form-input" type="date" value={movementFilters.from} onChange={(event) => setMovementFilters((current) => ({ ...current, from: event.target.value }))} />
          <input className="form-input" type="date" value={movementFilters.to} onChange={(event) => setMovementFilters((current) => ({ ...current, to: event.target.value }))} />
        </div>
        <div className="panel responsive-table-shell" style={{ padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
          <div style={{ overflow: 'auto', flex: 1 }}>
            <table className="data-table master-table-compact">
              <thead><tr><th>ID</th><th>Product ID</th><th>Product SKU</th><th>Location ID</th><th>Location</th><th>Batch ID</th><th>Type</th><th>Quantity</th><th>Reference Type</th><th>Reference ID</th><th>User ID</th><th>Notes</th><th>Occurred On</th></tr></thead>
              <tbody>
                {isLoading ? <LoadingRow colSpan={13} message="Loading movements..." /> : null}
                {!isLoading && error ? <ErrorRow colSpan={13} message={error} /> : null}
                {!isLoading && !error && movements.length === 0 ? <EmptyRow colSpan={13} message="No stock movements found." /> : null}
                {!isLoading && !error ? movements.map((item) => (
                  <tr key={item.id}>
                    <td className="mono">{item.id}</td>
                    <td className="mono">{item.productId}</td>
                    <td><span className="mono" style={{ color: 'var(--color-amber)' }}>{item.productSku}</span></td>
                    ...
                    <td><StatusBadge status={item.movementType} /></td>
                    <td className="amount-primary">{formatNumber(item.quantity)}</td>
  ...
  function renderTransfers() {
    return (
      <div className="responsive-master-detail" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 380px', gap: 16, flex: 1, minHeight: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0 }}>
          <div className="panel" style={{ padding: 12 }}>
            <SelectControl style={{ maxWidth: 220 }}>
              <select className="form-input" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
                <option value="">All statuses</option><option value="Draft">Draft</option><option value="Dispatched">Dispatched</option><option value="Received">Received</option><option value="Cancelled">Cancelled</option>
              </select>
            </SelectControl>
          </div>
          <div className="panel responsive-table-shell" style={{ padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
            <div style={{ overflow: 'auto', flex: 1 }}>
              <table className="data-table master-table-compact">
                <thead><tr><th>ID</th><th>No</th><th>Source ID</th><th>Source</th><th>Destination ID</th><th>Destination</th><th>Status</th><th>Lines</th><th>Dispatched On</th><th>Received On</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
                <tbody>
                  {isLoading ? <LoadingRow colSpan={11} message="Loading transfers..." /> : null}
                  {!isLoading && error ? <ErrorRow colSpan={11} message={error} /> : null}
                  {!isLoading && !error && transfers.length === 0 ? <EmptyRow colSpan={11} message="No transfers found." /> : null}
                  {!isLoading && !error ? transfers.map((transfer) => (
                    <tr key={transfer.id}>
                      <td className="mono">{transfer.id}</td>
                      <td><span className="mono" style={{ color: 'var(--color-amber)' }}>{transfer.transferNo}</span></td>
                      ...
                      <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                        <SmallButton onClick={() => viewTransfer(transfer.id)}>View</SmallButton>{' '}
                        {canCreateTransfers ? (
                          <>
                            <SmallButton onClick={() => transferAction(transfer.id, 'dispatch')}>Dispatch</SmallButton>{' '}
                            <SmallButton onClick={() => transferAction(transfer.id, 'receive')}>Receive</SmallButton>
                          </>
                        ) : null}
                      </td>
                    </tr>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {canCreateTransfers ? (
            <>
              <form className="panel" onSubmit={createTransfer} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                <h2 style={{ fontSize: 16, fontWeight: 650 }}>New Transfer</h2>
                <Field label="Source Location" required><LocationSelect value={transferForm.sourceLocationId} onChange={(value) => setTransferField('sourceLocationId', value)} /></Field>
                <Field label="Destination Location" required><LocationSelect value={transferForm.destinationLocationId} onChange={(value) => setTransferField('destinationLocationId', value)} /></Field>
                <Field label="Notes"><input className="form-input" value={transferForm.notes} onChange={(event) => setTransferField('notes', event.target.value)} /></Field>
                <button type="submit" className="button-primary" disabled={isSaving}>Create Transfer</button>
              </form>
 
              <form className="panel" onSubmit={addTransferLine} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                <h2 style={{ fontSize: 16, fontWeight: 650 }}>Add Transfer Line</h2>
                <Field label="Transfer" required><TransferSelect value={transferLineForm.transferId} onChange={(value) => setTransferLineField('transferId', value)} /></Field>
                <Field label="Product" required><ProductSelect value={transferLineForm.productId} onChange={(value) => setTransferLineField('productId', value)} /></Field>
                <Field label="Product SKU" required><input className="form-input" value={transferLineForm.productSku} onChange={(event) => setTransferLineField('productSku', event.target.value)} /></Field>
                <Field label="Source Batch ID" required><input className="form-input" value={transferLineForm.sourceBatchId} onChange={(event) => setTransferLineField('sourceBatchId', event.target.value)} /></Field>
                <Field label="Quantity" required><input className="form-input" type="number" min="0" step="0.01" value={transferLineForm.qty} onChange={(event) => setTransferLineField('qty', event.target.value)} /></Field>
                <button type="submit" className="button-primary" disabled={isSaving}>Add Line</button>
              </form>
            </>
          ) : (
            <PermissionNotice>Transfer create permission is required to create, dispatch, receive, cancel, or edit transfer lines.</PermissionNotice>
          )}
        </div>
      </div>
    )
      ...
      <div className="responsive-master-detail" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 380px', gap: 16, flex: 1, minHeight: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0 }}>
          <div className="panel" style={{ padding: 12 }}>
            <SelectControl style={{ maxWidth: 220 }}>
              <select className="form-input" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} style={{ appearance: 'none', backgroundImage: 'none', paddingRight: 36 }}>
                <option value="">All statuses</option><option value="Draft">Draft</option><option value="Counting">Counting</option><option value="Completed">Completed</option><option value="Cancelled">Cancelled</option>
              </select>
            </SelectControl>
          </div>
          <div className="panel responsive-table-shell" style={{ padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
            <div style={{ overflow: 'auto', flex: 1 }}>
              <table className="data-table master-table-compact">
                <thead><tr><th>ID</th><th>No</th><th>Location ID</th><th>Location</th><th>Status</th><th>Total Lines</th><th>Counted Lines</th><th>Completed On</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
                <tbody>
                  {isLoading ? <LoadingRow colSpan={9} message="Loading stocktakes..." /> : null}
                  {!isLoading && error ? <ErrorRow colSpan={9} message={error} /> : null}
                  {!isLoading && !error && stocktakes.length === 0 ? <EmptyRow colSpan={9} message="No stocktakes found." /> : null}
                  {!isLoading && !error ? stocktakes.map((stocktake) => (
                    <tr key={stocktake.id}>
                      <td className="mono">{stocktake.id}</td>
                      <td><span className="mono" style={{ color: 'var(--color-amber)' }}>{stocktake.stocktakeNo}</span></td>
                      <td className="mono">{stocktake.stockLocationId}</td>
                      <td>{locationName(stocktake.stockLocationId)}</td>
                      <td><StatusBadge status={stocktake.status} /></td>
                      <td>{stocktake.totalLines}</td>
                      <td>{stocktake.countedLines}</td>
                      <td>{formatDate(stocktake.completedOn)}</td>
                      <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                        <SmallButton onClick={() => viewStocktake(stocktake.id)}>View</SmallButton>{' '}
                        <SmallButton onClick={() => stocktakeAction(stocktake.id, 'start')}>Start</SmallButton>{' '}
                        <SmallButton onClick={() => stocktakeAction(stocktake.id, 'complete')}>Complete</SmallButton>
                      </td>
                    </tr>
                  )) : null}
                </tbody>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <form className="panel" onSubmit={createStocktake} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <h2 style={{ fontSize: 16, fontWeight: 650 }}>New Stocktake</h2>
            <Field label="Stock Location" required><LocationSelect value={stocktakeForm.stockLocationId} onChange={(value) => setStocktakeField('stockLocationId', value)} /></Field>
            <Field label="Notes"><input className="form-input" value={stocktakeForm.notes} onChange={(event) => setStocktakeField('notes', event.target.value)} /></Field>
            <button type="submit" className="button-primary" disabled={isSaving}>Create Stocktake</button>
          </form>
 
          <form className="panel" onSubmit={addStocktakeLine} style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <h2 style={{ fontSize: 16, fontWeight: 650 }}>Add Count Line</h2>
            <Field label="Stocktake" required><StocktakeSelect value={stocktakeLineForm.stocktakeId} onChange={(value) => setStocktakeLineField('stocktakeId', value)} /></Field>
            <Field label="Product" required><ProductSelect value={stocktakeLineForm.productId} onChange={(value) => setStocktakeLineField('productId', value)} /></Field>
            <Field label="Product SKU" required><input className="form-input" value={stocktakeLineForm.productSku} onChange={(event) => setStocktakeLineField('productSku', event.target.value)} /></Field>
            <button type="submit" className="button-primary" disabled={isSaving}>Add Line</button>
          </form>
        </div>
      </div>
      ...
      <div className="panel responsive-table-shell" style={{ padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
          <div>
            ...
            <p style={{ color: 'var(--color-text-muted)', fontSize: 12 }}>{locationName(selectedTransfer.sourceLocationId)} to {locationName(selectedTransfer.destinationLocationId)}</p>
            <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(4, minmax(140px, 1fr))', gap: 8, color: 'var(--color-text-muted)', fontSize: 12 }}>
              <span>ID: <span className="mono">{selectedTransfer.id}</span></span>
              <span>Created By: <span className="mono">{selectedTransfer.createdByUserId || '-'}</span></span>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {canCreateTransfers ? <SmallButton onClick={() => transferAction(selectedTransfer.id, 'cancel')}>Cancel</SmallButton> : null}
            <StatusBadge status={selectedTransfer.status} />
          </div>
        </div>
        <div style={{ overflow: 'auto', flex: 1 }}>
          <table className="data-table master-table-compact">
            <thead><tr><th>ID</th><th>Product ID</th><th>Product SKU</th><th>Source Batch ID</th><th>Batch No</th><th>Qty</th><th>Unit Cost Smallest</th><th>Selling Price</th><th>MRP</th><th>Expiry</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
            <tbody>
              {(selectedTransfer.lines || []).length === 0 ? <EmptyRow colSpan={11} message="No transfer lines added." /> : null}
              {(selectedTransfer.lines || []).map((line) => (
                <tr key={line.id}>
                  <td className="mono">{line.id}</td>
                  <td className="mono">{line.productId}</td>
                  <td><span className="mono" style={{ color: 'var(--color-amber)' }}>{line.productSku}</span></td>
                  <td className="mono">{line.sourceBatchId}</td>
                  <td>{line.batchNo || '-'}</td>
                  <td className="amount-primary">{formatNumber(line.qtyToTransfer)}</td>
                  ...
                  <td style={{ textAlign: 'right' }}>{canCreateTransfers ? <SmallButton onClick={() => removeTransferLine(line.id)}>Remove</SmallButton> : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="panel responsive-table-shell" style={{ padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0, flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
          <div>
            ...
            <p style={{ color: 'var(--color-text-muted)', fontSize: 12 }}>{locationName(selectedStocktake.stockLocationId)}</p>
            <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(4, minmax(140px, 1fr))', gap: 8, color: 'var(--color-text-muted)', fontSize: 12 }}>
              <span>ID: <span className="mono">{selectedStocktake.id}</span></span>
              <span>Created By: <span className="mono">{selectedStocktake.createdByUserId || '-'}</span></span>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {canManageStocktakes ? <SmallButton onClick={() => stocktakeAction(selectedStocktake.id, 'cancel')}>Cancel</SmallButton> : null}
            <StatusBadge status={selectedStocktake.status} />
          </div>
        </div>
        <div style={{ overflow: 'auto', flex: 1 }}>
          <table className="data-table master-table-compact">
            <thead><tr><th>ID</th><th>Product ID</th><th>Product SKU</th><th>Expected</th><th>Counted</th><th>Variance</th><th>Is Counted</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
            <tbody>
              {(selectedStocktake.lines || []).length === 0 ? <EmptyRow colSpan={8} message="No stocktake lines added." /> : null}
              {(selectedStocktake.lines || []).map((line) => (
                <tr key={line.id}>
                  <td className="mono">{line.id}</td>
                  <td className="mono">{line.productId}</td>
                  <td><span className="mono" style={{ color: 'var(--color-amber)' }}>{line.productSku}</span></td>
                  <td className="amount">{formatNumber(line.expectedQty)}</td>
                  <td style={{ minWidth: 150 }}>
                    <input className="form-input" type="number" min="0" step="0.01" value={countValues[line.id] ?? ''} onChange={(event) => setCountValues((current) => ({ ...current, [line.id]: event.target.value }))} style={{ height: 32 }} />
                  </td>
                  <td className="amount-primary">{line.varianceQty == null ? '-' : formatNumber(line.varianceQty)}</td>
                  <td>{formatBoolean(line.isCounted)}</td>
                  <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                    {canManageStocktakes ? (
                      <>
                        <SmallButton onClick={() => saveCount(line.id)}>Save Count</SmallButton>{' '}
                        <SmallButton onClick={() => removeStocktakeLine(line.id)}>Remove</SmallButton>
                      </>
                    ) : '-'}
                  </td>
                </tr>
