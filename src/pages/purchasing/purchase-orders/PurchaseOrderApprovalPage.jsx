...
export default function PurchaseOrderApprovalPage() {
  ...
  // Filter orders client-side by date range and search query
  const filteredOrders = useMemo(() => {
    ...
    const filtered = rawOrders.filter((po) => {
      ...
      return matchFrom && matchTo && matchesSearch
    })
  ...
  }, [rawOrders, searchFromDate, searchToDate, search])
  ...
  function clearFilters() {
    setSearch('')
    setFilterFromDate(dayjs().subtract(30, 'day').format('YYYY-MM-DD'))
    setFilterToDate(dayjs().format('YYYY-MM-DD'))
    setFilterSupplier('')
    setSearchFromDate(dayjs().subtract(30, 'day').format('YYYY-MM-DD'))
    setSearchToDate(dayjs().format('YYYY-MM-DD'))
    setSearchSupplier('')
    setSelectedPoId(null)
  }
                ...
                {canApprove ? (
                  <div style={{ display: 'flex', gap: 10 }}>
                    <ConfirmDialog
                      ...
                      description="This purchase order will leave the approval queue and return as rejected. Make sure your rejection remark explains the decision clearly."
                      details={selectedPoDetail?.poNumber}
