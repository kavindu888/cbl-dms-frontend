...
const mockVehicles = [
  { reg: 'WP-KH-3421', type: 'Lorry 3T',   driver: 'K.Bandara',  status: 'ON ROUTE',     route: 'Colombo North', lastUpdate: '14:21', odometer: 128450, fuel: 68 },
  { reg: 'WP-GA-7823', type: 'Lorry 1.5T', driver: 'R.Fernando', status: 'ON ROUTE',     route: 'Gampaha',       lastUpdate: '14:15', odometer: 94200,  fuel: 45 },
  { reg: 'WP-MB-4521', type: 'Van',         driver: '—',          status: 'IN WAREHOUSE', route: '—',             lastUpdate: '08:00', odometer: 62100,  fuel: 90 },
  { reg: 'WP-AB-1234', type: 'Lorry 5T',   driver: '—',          status: 'MAINTENANCE',  route: '—',             lastUpdate: '06:30', odometer: 210800, fuel: 20 },
]
...
export default function FleetOverviewPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold" style={{ color: 'var(--color-text-primary)' }}>Fleet</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'On Route',    value: onRoute,    icon: <Truck className="h-5 w-5" />,    color: 'var(--color-teal)' },
          { label: 'In Warehouse',value: inWarehouse,icon: <MapPin className="h-5 w-5" />,  color: 'var(--color-blue)' },
          { label: 'Maintenance', value: maintenance, icon: <Wrench className="h-5 w-5" />, color: 'var(--color-danger)' },
          ...
          <div key={label} className="panel p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-2xl font-bold mono" style={{ color }}>{value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {mockVehicles.map((v) => (
          ...
          >
            ...
            <p className="mono font-bold text-lg" style={{ color: 'var(--color-text-primary)' }}>{v.reg}</p>
            <p className="text-xs mt-1" style={{ color: 'var(--color-text-muted)' }}>{v.type}</p>
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="mono" style={{ color: 'var(--color-text-muted)' }}>{v.odometer.toLocaleString()} km</span>
              </div>
              <div className="flex justify-between text-xs">
                <span style={{ color: 'var(--color-text-dim)' }}>Fuel</span>
                <span className="mono" style={{ color: v.fuel < 30 ? 'var(--color-danger)' : 'var(--color-text-muted)' }}>{v.fuel}%</span>
              </div>
            </div>
